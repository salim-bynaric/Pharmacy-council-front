...
const StudentGetStarted = () => {
    let history = useHistory();
    const { setCurrentUser } = useContext(UserContext);
    const { setShow, setMsg } = useContext(ShowContext);
    const [registrationNo, setRegisrationNo] = useState('');
    ...
    return (
        <>
            ...
            <div className="wrap-input100 validate-input m-b-16" data-validate="Phone is required">
                <input className="input100" type="registrationNo" name="registrationNo" placeholder="Registration Number..." value={registrationNo} maxLength={20} onChange={(e) => { setRegisrationNo(e.target.value) }} /></div>
