...
const StudentAttendanceReport = () => {
    ...
    useEffect(() => {
        if (currentUser !== undefined && currentUser !== null) {
            getProctorDateSlot(setDateSlot, currentUser, setShow, setMsg);
        }
    }, [currentUser]);
            ...
            <div>
                <div className="container-fluid">
                    ...
                    <ol className="breadcrumb mb-4">
                        <li className="breadcrumb-item active">Student Attendance Report</li>
                    </ol>
...
function printAttendancePdf(proctorData,currentUser,dateSlot)
{
    ...
    {
        doc.text("Student Attendance Report", 60, 7);
    }
    else
    {
...
function printPdf(proctorData,currentUser,dateSlot)
{
    ...
    if(currentUser.role === 'ADMIN')
    {
        doc.text("Date Slot wise Student Allocation Report", 60, 7);
    }
    else
    {
...
async function getAllocatedStudents(currentUser, dateSlot, setShow, setMsg, setProctorData) {
    await API.get('/proctorAllocation', { params: { "type": "allByDateSlot", "dateSlot": dateSlot, "instId": currentUser.inst_id } })
        .then((res) => {
...
async function getProctorDateSlot(setDateSlot, currentUser, setShow, setMsg) 
{
    await API.get('/proctor/dateSlots', { params: { "type": "all"} })
        .then((res) => {
            if (res.data.status === 'success') {
