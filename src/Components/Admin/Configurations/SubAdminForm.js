...
const validationSchema = Yup.object({
    ...
    name: Yup.string()
        .required("Name of Sub-Admin is Required."),
    orgName: Yup.string()
        .required("Organization Name of Sub-Admin is Required."),
    email: Yup.string()
        .required("Email of Sub-Admin is Required.")
        .email(),
    mobile: Yup.number()
        .required("Mobile Number of Sub-Admin is Required."),
    password: Yup.string()
...
async function registerAdmin(instId, name, orgName, email, mobile, password, setLoading, setMyMsg, setMyList, myList) 
{
    ...
    setLoading(true);
    await API.post('/user', { 'type': 'SUBADMIN', 'instId':instId ,'name': name, 'orgName': orgName, 'email': email, 'mobile': mobile, 'password': password })
        .then(function (res) {
            setLoading(false);
