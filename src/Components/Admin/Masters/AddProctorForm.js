...
const AddProctorForm = (props) => {
    ...
    return (
        ...
        {
            await saveProctor(values,setShow,setMsg,currentUser,props.setMyList,props.myList);
                            ...
                            <div className="card-body">
                                    ...
                                    <div className="form-group">
                                        <div className="col-lg-12 row">
                                            ...
                                            <div className="col-lg-8">
                                                <input type="text" id="name" name="name" onChange={handleChange} value={values.name} onBlur={handleBlur} className="form-control" placeholder="Enter Proctor Name..." />
...
async function saveProctor(values,setShow,setMsg,currentUser,setMyList,myList)
{
    ...
    await API.post('/user',{'type':'proctor','username':username,'name':name,'instId':inst,'mobile':mobile,'email':email,'password':password})
    .then((res) => 
    {
        if(res.data.status === 'success')
        {
            setShow(true);
            setMsg('Proctor Added Successfully...');
            setMyList(!myList);
        }
    })
    ...
    {
        setShow(true);
        setMsg('Problem Adding Proctor to Database...');
    });
}
