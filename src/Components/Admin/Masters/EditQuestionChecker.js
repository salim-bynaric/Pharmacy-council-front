...
const EditQuestionChecker = (props) => {
    ...
    return (
        ...
        {
            await updateQuestionChecker(values,id,setShow,setMsg,currentUser,props.setMyList,props.myList,setEdit);
        }}
                ...
                return (
                    <Modal show={openEdit} onHide={handleClose} backdrop="static" size="lg">
                        <Modal.Header closeButton style={{backgroundColor:"OliveDrab",color:"white"}}>
                            <Modal.Title>Edit Question Checker</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
...
async function updateQuestionChecker(values,id,setShow,setMsg,currentUser,setMyList,myList,setEdit)
{
    ...
    await API.put('/user/'+id,{'type':'checker','username':username,'name':name,'instId':inst,'mobile':mobile,'email':email})
    .then((res) => 
    {
        if(res.data.status === 'success')
        {
            setShow(true);
            setMsg('Checker Updated Successfully...');
            setMyList(!myList);
            setEdit(false);
        }
    })
    ...
    {
        setShow(true);
        setMsg('Problem Updating Checker...');
    });
}
...
export default EditQuestionChecker;