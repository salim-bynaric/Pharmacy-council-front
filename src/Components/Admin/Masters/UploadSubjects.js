...
const UploadSubjects = (props) => {
    ...
    const formik = useFormik({
        ...
        {
            setMyMsg('');
            await UpSubjects(values.file,setMyMsg,props.myList,props.setMyList,setLoading,channel);
...
function download()
{
    ...
    const response = {
        file: serverPath+'standardFormats/subjectMaster.xlsx',
      };
    window.open(response.file);
}

async function UpSubjects(file,setMyMsg,myList,setMyList,setLoading,channel)
{
    setLoading(true);
    let fd = new FormData();
    fd.append("file", file);
