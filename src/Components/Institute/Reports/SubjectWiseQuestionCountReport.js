...
const SubjectWiseQuestionCountReport = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      getSubjectWiseQuestionData(setShow, setMsg, currentUser, setData);
    }
  }, [currentUser]);
...
function getHeader()
{
    let myHeader = [
        ...
        { text: 'Subject', dataField: 'paperCode',filter: textFilter()},
        { text: 'Topic', dataField: 'topic',filter: textFilter()},
        { text: 'Subtopic', dataField: 'subtopic',filter: textFilter()},
        { text: 'Marks', dataField: 'marks',filter: textFilter()},
        { text: 'Total Questions', dataField: 'total'},
    ];
    return myHeader;
}

async function getSubjectWiseQuestionData(setShow, setMsg, currentUser, setData) {
  let param = null;
  ...
  await API.get("/report/subjectWiseQuestionCount", param)
    .then((res) => {
...
export default SubjectWiseQuestionCountReport;
