...
import PreCartModal from './PreCartModal';
...
const ExamCardPurchase = ({ exams, purchased, subCategory,handleClose1,useSubcategory=true,cartUrl='/cart' }) => {
    let cartArray = [];
    ...
    return (
        subCategory ?
            <>
                                                        ...
                                                        <div className="card-body">
                                                            ...
                                                            <center key={index}>
                                                                {/*<button className='add_cart' onClick={() => {
                                                                    ...
                                                                    setCart(cartArray);
                                                                    lable == 'Go To Cart' ? history.push(cartUrl) : setOpenPreCart(true)                                                                 
                                                                }} style={{ "fontSize": "12px" }}>{lable}</button>*/}
                                                                 <button className='btn_Purchase' onClick={() => {
                                                                    cartArray.push(exam);
                                                                    cartArray = removeDuplicate(cartArray, 'paper_code');
                                                                    localStorage.setItem('myExamCart', undefined);
                                                                    localStorage.setItem('myExamCart', JSON.stringify(cartArray));
                ...
                {
                    clickedExam ?
                        <PreCartModal show={openPreCart} handleClose={handleClose} handleClose1={handleClose1} exam={clickedExam} cartUrl={cartUrl}/>
                    : null
                }
            </>
        : null
